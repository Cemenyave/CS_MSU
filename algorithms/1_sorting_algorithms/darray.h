...
public:
  ...
  ///
  void insert(const T * const &data, const unsigned int len, const unsigned int position)
  {
    int allocate = len - (this->allocated - this->elemcount);
  ...
  ///
  void append(const T * const &data, const unsigned int len)
  {
  ...
  ///
  void prepend(const T * const &data, unsigned int len)
  {
    insert(data, len, 0);
  }
  ...
  ///
  void resize(const unsigned int new_size, const T placeholder = T())
  {
  ...
  ///
  void extend(const unsigned int size = DEAFULT_ALLOCATION_SIZE)
  {
    this->allocated += size;
    if (!this->dataptr) {
